   ...
   public function index(request $request){
     ...
     //   $kunci = [];
       ...
       // $jawaban=[];
        ...
        $ceks=db::table('tb_bidang')->where('id_bidang','1')->get();
        foreach ($ceks as $data) {
            $statussaintek=$data->status;
        }
        $ceks=db::table('tb_bidang')->where('id_bidang','2')->get();
        foreach ($ceks as $data) {
            $statussoshum=$data->status;
        }

         if(Auth::user()->akses == 'ALL') {
            
               return view('tryout.pilih_akses');
       	}else if(Auth::user()->akses == 'SAINTEK'){
        		$request->session()->put('bidang','1');
       			if ($statussaintek=="OFF") {
                      return view('tryout.tolak-tryout');
                }
                  return view('tryout.mulaitryout');
                
      	}else{
        	$request->session()->put('bidang','2');		
            if ($statussaintek=="OFF") {
                  return view('tryout.tolak-tryout');
                }
                  return view('tryout.mulaitryout');
